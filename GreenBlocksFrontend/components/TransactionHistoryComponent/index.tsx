...
export default function TransactionHistoryComponent({ walletAddress, chain, limit }: TransactionHistoryProps) {
  const [isLoading, setIsloading] = useState<boolean>(false);
  const [outgoingTransactionsHistory, setOutgoingTransactionsHistory] =   useState<Transaction[] | null>(null);
  const [incomingTransactionsHistory, setIncomingTransactionsHistory] =   useState<Transaction[] | null>(null);
  const [isIncomingTransactionsView, setIsIncomingTransactionsView] =     useState<boolean>(true);
  const [outgoingAssetTransferPageKey, setOutgoingAssetTransferPageKey] = useState<string | null>(null);
  const [incomingAssetTransferPageKey, setIncomingAssetTransferPageKey] = useState<string | null>(null);
  ...
  const getTransactionHistory = async () => {
    ...
    if (isConnected || walletAddress) {
      const transactions = await fetch("/app/transactions/getTransactionHistory", {
        method: "POST",
        body: JSON.stringify({
      ...
      if (res.incomingTransactionsHistory?.length) {
        if (incomingAssetTransferPageKey) {
          setIncomingTransactionsHistory((prev) => {
            ...
            return res.incomingTransactionsHistory;
          });
        } else {
        ...
        if (res.incomingAssetTransferPageKey?.length) {
          ...
          setIncomingAssetTransferPageKey(null);
        }
      }
      if (res.outgoingTransactionsHistory?.length) {
        if (outgoingAssetTransferPageKey) {
          setOutgoingTransactionsHistory((prev) => {
            ...
            return res.outgoingTransactionsHistory
          });
        } else {
        ...
        if (res.outgoingAssetTransferPageKey?.length) {
          ...
          setOutgoingAssetTransferPageKey(null);
        }
      }
      setIsloading(false);
    }
